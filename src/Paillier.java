...
public class Paillier {
	 ...
	 */
	public Paillier() {
		KeyGeneration(1024, 160);
	}
	
	// Build the Paillier system by given N, G, and Lambda
	public Paillier(String sN, String sG, String sLambda){
		n = new BigInteger(sN);
		g = new BigInteger(sG);
		lambda = new BigInteger(sLambda);
	...
	// Build the Paillier system by given N, G, and Lambda
	public Paillier(BigInteger sN, BigInteger sG, BigInteger sLambda){
		n = sN;
		g = sG;
		lambda = sLambda;
	...
	public void KeyGeneration(int bitLengthVal, int certainty) {
		...
		//g = new BigInteger("2");
		g = new BigInteger(160, new Random());
	...
	/** 
	 * Encrypt the message by public keys <n, g> in Paillier cryptosystem  
	 * */
	public static BigInteger PublicPaillierEncryption(BigInteger n, BigInteger g, BigInteger m){
	...
	static public BigInteger PublicMoreEncryption(BigInteger n, BigInteger g, BigInteger m) {
		int bitLength = n.bitLength();
		BigInteger nsquare = n.multiply(n);
		BigInteger r = new BigInteger(bitLength, new Random());

		return m.multiply(r.modPow(n, nsquare)).mod(nsquare);
	}
